...
- **🎖️ Achievements & Badges System:** 29 unlockable trophies with animated toast notifications, sound fanfares, progress bar tracking, and dedicated modal card (including `⚡ Combo King`, `⛶ Immersive Solver`, `🎨 Theme Connoisseur`, `🎭 Silhouette Artist`, `🔮 Prism Perfectionist`, `🛡️ Guardian Solver`, and `🌌 Celestial Navigator`).
...
- **🎨 16 Themed Color Palettes:** Choose from 16 curated atmospheric themes (Cyber Midnight, Sunset Synth, Deep Emerald, Retro Sepia, Volcanic Ruby, Sakura Blossom, Neon Cyberpunk, ❄️ Nordic Frost, ☕ Espresso Roast, 🔮 Cosmic Violet, 🌊 Ocean Abyss, 🌅 Solar Flare, 🌌 Aurora Borealis, 🍯 Golden Amber, 🔮 Mystic Amethyst, ⚡ Cyber Lime) or design your own with the Custom Theme Studio.
...
- **✨ Geometric Cutout Shapes & Faceted Polygons:** Choose from 17 unique piece cutout styles: Classic Square, Pill, Hexagon, Diamond, Cyber Glow, Wave Curves, Classic Tabs, Starburst (12-point star polygon), Gem Octagon (faceted jewel cut), Clover Petals (🍀 4-lobed emerald bloom), Puzzle Gear (⚙️ mechanical interlocking cog), Prism Shard (🔮 multi-faceted crystalline jewel), Botanical Leaf (🍃 curved organic leaf silhouette), Sparkle Star (✨ 4-point diamond star), Knight Shield (🛡️ medieval crest aegis), Crescent Moon (🌙 celestial crescent arc), and Starlight Cross (✚ orthogonal heraldic cross)!
- **🎭 13 Board Silhouette Masks:** Shape the overall puzzle canvas with dynamic silhouette clipping: Classic Grid, Circle, Diamond, Medieval Shield, Valentine Heart, 5-Point Star (⭐), Grand Hexagon (⬡), Whimsical Cloud (☁️), Octagon Shield (🛑), Brilliant Jewel (💎), Delta Triangle (🔺), Crescent Moon (🌙), and Hourglass (⏳)!
...
- **📅 Daily Seeded Challenge & Monthly Calendar History:**
  ...
  - **Interactive Monthly Calendar:** Full month view with month/year navigation (`◀` / `▶`), today jump button, daily star ratings, and completion checkmarks.
  - **Archive Challenge Replay:** Click and solve any past date's puzzle to catch up on missed days and earn archive completion bonuses.
  - **Streak & Milestone Badges:** Tracks current consecutive streaks and all-time best streaks (`🔥`), with 5 unlockable daily milestone badges (🥉 Streak Pioneer, 🥈 Week Warrior, 🥇 Fortnight Master, 💎 Monthly Legend, 🧭 Archive Explorer) integrated with the global trophy system.
...
- **📱 PWA Offline Support & App Shortcuts:** Built with Web App Manifest (`manifest.json`) and Service Worker (`sw.js` v9) for 100% offline gameplay, featuring App Shortcuts and URL deep-linking (`?mode=daily`, `?mode=collage`, `?mode=motion`, `?mode=camera`).
