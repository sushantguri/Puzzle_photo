...
- **🎖️ Achievements & Badges System:** 28 unlockable trophies with animated toast notifications, sound fanfares, progress bar tracking, and dedicated modal card (including `⚡ Combo King`, `⛶ Immersive Solver`, `🎨 Theme Connoisseur`, `🎭 Silhouette Artist`, `🔮 Prism Perfectionist`, and `🛡️ Guardian Solver`).
...
- **🎨 14 Themed Color Palettes:** Choose from 14 curated atmospheric themes (Cyber Midnight, Sunset Synth, Deep Emerald, Retro Sepia, Volcanic Ruby, Sakura Blossom, Neon Cyberpunk, ❄️ Nordic Frost, ☕ Espresso Roast, 🔮 Cosmic Violet, 🌊 Ocean Abyss, 🌅 Solar Flare, 🌌 Aurora Borealis, 🍯 Golden Amber) or design your own with the Custom Theme Studio.
...
- **✨ Geometric Cutout Shapes & Faceted Polygons:** Choose from 15 unique piece cutout styles: Classic Square, Pill, Hexagon, Diamond, Cyber Glow, Wave Curves, Classic Tabs, Starburst (12-point star polygon), Gem Octagon (faceted jewel cut), Clover Petals (🍀 4-lobed emerald bloom), Puzzle Gear (⚙️ mechanical interlocking cog), Prism Shard (🔮 multi-faceted crystalline jewel), Botanical Leaf (🍃 curved organic leaf silhouette), Sparkle Star (✨ 4-point diamond star), and Knight Shield (🛡️ medieval crest aegis)!
- **🎭 11 Board Silhouette Masks:** Shape the overall puzzle canvas with dynamic silhouette clipping: Classic Grid, Circle, Diamond, Medieval Shield, Valentine Heart, 5-Point Star (⭐), Grand Hexagon (⬡), Whimsical Cloud (☁️), Octagon Shield (🛑), Brilliant Jewel (💎), and Delta Triangle (🔺)!
...
- **📱 PWA Offline Support & App Shortcuts:** Built with Web App Manifest (`manifest.json`) and Service Worker (`sw.js` v8) for 100% offline gameplay, featuring App Shortcuts and URL deep-linking (`?mode=daily`, `?mode=collage`, `?mode=motion`, `?mode=camera`).
